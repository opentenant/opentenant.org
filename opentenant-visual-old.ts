...
class VisualCompiler {
  ...
  private layout(): { width: number; height: number; maxDepth: number } {
    ...
    // 1. assign x by leaf packing & parent centering
    let cursorX = 0;
    const assignX = (n: VNode): number => {
    ...
    // 2. assign y by depth bands (each band sized to the tallest card in that depth)
    const nodesByDepth = new Map<number, VNode[]>();
    ...
    for (let d = 0; d <= maxDepth; d++) {
      const bandNodes = nodesByDepth.get(d) || [];
      if (bandNodes.length === 0) continue;
      const maxHeight = Math.max(...bandNodes.map((n) => n.h));
      // Place all nodes of this depth at the same y
      const yPos = yCursor + DEPTH_BAND_PADDING;
      for (const n of bandNodes) {
        n.y = yPos;
      }
      yCursor += maxHeight + DEPTH_BAND_PADDING * 2;
    }
  ...
  private emit(layout: { width: number; height: number; maxDepth: number }): string {
    ...
    const legend = [
      ...
      { c: "#22d3ee", t: "composite edge" },
    ];
    let legX = width - MARGIN - legend.length * 130;
    legend.forEach((l, i) => {
      const lx = legX + i * 130;
    ...
    for (let d = 0; d <= maxDepth; d++) {
      const bandNodes = nodesByDepth.get(d) || [];
      if (bandNodes.length === 0) continue;
      const minY = Math.min(...bandNodes.map((n) => n.y - DEPTH_BAND_PADDING));
      const maxY = Math.max(...bandNodes.map((n) => n.y + n.h + DEPTH_BAND_PADDING));
      const bandHeight = maxY - minY;
      out.push(`<rect x="0" y="${minY}" width="${width}" height="${bandHeight}" fill="#ffffff" opacity="${d % 2 === 0 ? 0.015 : 0.04}"/>`);
      out.push(`<text x="12" y="${minY + 18}" fill="${pal(d).accent}" font-size="11" opacity="0.8">DEPTH ${d}${d === 0 ? " · OPERATOR" : ` · RESALE HOP ${d}`}</text>`);
    }
    ...
    for (const n of this.nodes) {
      for (const t of n.templates) {
        for (const comp of t.composes ?? []) {
          ...
          const ancIdx = anc.templates.findIndex((tt) => tt.id === comp.template);
          if (ancIdx === -1) continue;
          const x1 = anc.x + anc.w, y1 = anc.y + 60 + ancIdx * 16 + 8;
