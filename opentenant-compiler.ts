  ...
  private validate(): void {
    ...
    for (const [id] of this.nodes) {
      ...
      for (const tenant of this.nodes.get(id)!.tenants ?? []) {
        ...
        if (tenant.enabled === false || tenant.placement == null) continue;

        if (!tenant.placement.cloud || !tenant.placement.region) {

          this.fail("tenant_placement_complete", this.pathOf(id),

            `Tenant '${tenant.id}' placement must include both cloud and region`);
    ...
    for (const rule of this.blueprint.validation?.rules ?? []) {
      ...
      const compiledIn = new Set([
        ...
        "vault_namespace_matches_tree_path", "no_orphan_nodes", "tenant_placement_complete",
    ...
    return JSON.stringify({
      ...
      metadata: {
        ...
        labels: {
          ...
          ...(node.tenants?.length === 1 && node.tenants[0].placement?.cloud ? { "opentenant.org/cloud": node.tenants[0].placement.cloud, "opentenant.org/region": node.tenants[0].placement.region ?? "" } : {}),
  ...
  private emitBillingLedger(node: NodeDef, path: string, depth: number): string {
    ...
    for (const t of node.tenants ?? []) {
      ...
      for (const sub of t.subscriptions ?? []) {
        ...
        const placement = t.placement ? `,cloud=${t.placement.cloud ?? ""},region=${t.placement.region ?? ""}` : "";
        lines.push(`ledger_entry,tenant=${t.id},template=${sub.template},royalty_per_seat=${royalty}${placement}`);
